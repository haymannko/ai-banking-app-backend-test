    ...
    try:
        ...
        with sentry_sdk.start_transaction(op="task", name=f"OCR-{class_name}"):
            if class_name == "passport":
                with sentry_sdk.start_span(op="preprocess", description="Preprocess Passport"):
                    gray = ocr.preprocess_image_for_passport_ocr()

                with sentry_sdk.start_span(op="model", description="Passport OCR Model"):
                    result = ocr.passport_ocr_model(gray)
                    os.remove(temp_file) 

            elif class_name == "licence":
                with sentry_sdk.start_span(op="preprocess", description="Preprocess Licence"):
